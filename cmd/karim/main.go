...
func printHelp() {
	...
	fmt.Println("    --memory <N>MB             Memory limit (e.g. 64MB, 256MB)")
	fmt.Println("    --restart <policy>         Restart policy: always, on-failure, never (default: on-failure)")
	fmt.Println("    --seccomp <profile>        Seccomp profile: app-default, network, unrestricted")
...
func main() {
	...
	case "apply":
		if len(args) < 2 {
			fmt.Fprintf(os.Stderr, "Error: missing TOML file path. Usage: karim apply <service.toml>\n")
			os.Exit(1)
		}
...
// runApply reads a local TOML service config file and hot-loads it into the running MicroVM
// via the AF_VSOCK control plane — no reboot required.
//
// Usage: karim apply <service.toml>
func runApply(target, tomlPath string) {
	tomlBytes, err := os.ReadFile(tomlPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: failed to read TOML file %q: %v\n", tomlPath, err)
		os.Exit(1)
	}
	...
	req := vsockd.RPCRequest{
		ID:      "apply-1",
		Command: "load_service",
		Payload: string(tomlBytes),
	}
	...
	if !resp.Success {
		fmt.Fprintf(os.Stderr, "Error from guest supervisor: %s\n", resp.Error)
		os.Exit(1)
	}

	fmt.Printf("[karim-cli] %v\n", resp.Data)
	fmt.Printf("[karim-cli] Service from %q is now running on %s\n", filepath.Base(tomlPath), target)
}

// runRun constructs an inline TOML service spec from CLI flags and sends it to the
// running MicroVM guest supervisor over AF_VSOCK — no reboot required.
//
// Usage: karim run --name <svc> --exec <cmd> [--memory <N>MB] [--restart <policy>] [--seccomp <profile>]
func runRun(target string, cmdArgs []string) {
	fs := flag.NewFlagSet("run", flag.ExitOnError)
	name := fs.String("name", "", "Service name (required)")
	execCmd := fs.String("exec", "", "Binary path to execute inside the guest rootfs (required)")
	memory := fs.String("memory", "", "Memory limit, e.g. 64MB or 256MB (optional)")
	restart := fs.String("restart", "on-failure", "Restart policy: always, on-failure, never")
	seccomp := fs.String("seccomp", "app-default", "Seccomp profile: app-default, network, unrestricted")
	...
	if *name == "" {
		fmt.Fprintf(os.Stderr, "Error: --name is required. Usage: karim run --name <svc> --exec <cmd>\n")
		os.Exit(1)
	}
	if *execCmd == "" {
		fmt.Fprintf(os.Stderr, "Error: --exec is required. Usage: karim run --name <svc> --exec <cmd>\n")
		os.Exit(1)
	}

	// Build a minimal TOML spec from CLI flags
	var toml strings.Builder
	toml.WriteString("# Generated by karim run\n")
	fmt.Fprintf(&toml, "name = %q\n", *name)
	fmt.Fprintf(&toml, "exec = %q\n", *execCmd)
	fmt.Fprintf(&toml, "restart = %q\n", *restart)
	fmt.Fprintf(&toml, "seccomp_profile = %q\n", *seccomp)
	if *memory != "" {
		fmt.Fprintf(&toml, "memory_limit = %q\n", *memory)
	}
